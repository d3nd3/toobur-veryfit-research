...
- **TOOBUR / Band 8: GATT error 133, `CheckPermission: _bluetooth code = 10` (e.g. ColorOS / OPPO A5 2020)** — TOOBUR uses **legacy `connectGatt` only** (logcat: `connectGatt legacy`). After **turning Bluetooth on**, wait **~8 s** (other phones) or **~18 s** (**OPPO / Realme / OnePlus**: `OemBleHeuristics`), or keep Gadgetbridge open for automatic **post-`STATE_ON` deferral** (`BluetoothStackSettleTracker`). On that OEM family, TOOBUR also adds a **non-blocking ~2.5 s delay before each connect attempt** and a longer **first auto-reconnect** delay. Cold-start right after enabling BT may still need a manual pause. Confirm **Bluetooth / nearby devices** permissions. Details: [`gadgetbridge/README_TOOBUR.md`](gadgetbridge/README_TOOBUR.md).
- **“Waiting for device scan” forever on Android 10 / API 29–30** — ColorOS scan-before-connect used `ScanFilter.Builder#setDeviceAddress`, which exists only from **API 31**. On older OS versions the filtered scan never matched, so there was no `onScanResult`. **Fix:** `BLEScanService` uses an **unfiltered scan + software address allowlist** when `SDK_INT < 31`; you should see log `API … < 31 — scan-before-connect using unfiltered BLE scan + software address match`.

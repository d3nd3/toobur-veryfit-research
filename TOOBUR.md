...
| | 0xA0 | Live data (`IDOGetLiveData` / `CMD_KEY_GET_LIVE_DATA`) | (none) → **19-byte** reply on **0x0AF7**: `02 A0` + **steps** u32 LE + **active_minutes** u32 LE (day uptime / “active time” minutes — *not* kcal) + **distance** u32 LE (often metres; confirm) + **reserved** u32 LE (often `0`) + **latest_HR** u8 (e.g. `0x34` = 52 bpm; `0` = none). Example: `02 a0 06 00 00 00 18 00 00 00 03 00 00 00 00 00 00 00 34` → steps=6, active_min=24, distance=3, HR=52. |
